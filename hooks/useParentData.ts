...
export function useParentData() {
    ...
    const fetchMyGoalies = async (showLoading = true) => {
        ...
        const currentId = emailToSearch || localId;
        ...
        // Prevent redundant fetches if the ID hasn't changed
        if (currentId === lastFetchedId.current && !showLoading) return;
        ...
        try {
            ...
            if ((!rosterData || rosterData.length === 0) && (localId || isDevAdmin)) {
                if (['GC-PRO-01', 'GC-8001', 'GC-PRO-HKY', 'GC-DEMO-01'].includes(localId || '') || isDevAdmin) {
                    rosterData = [{
                        id: DEMO_IDS.PRO_1,
                ...
                } else if (localId && ['GC-PRO-LAX', 'GC-8002'].includes(localId)) {
                    rosterData = [{
                        id: DEMO_IDS.PRO_2,
    ...
    }, [auth.userEmail, auth.localId, auth.loading]);
