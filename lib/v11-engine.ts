  ...
  calculateGoalieIndex(context: GoalieContext, isTrainingCompletedToday: boolean) {
    ...
    return {
      ...
      breakdown: {
        ...
        status: total > 90 ? 'MASTERS' : total > 70 ? 'ADVANCED' : 'DEVELOPING'
      }
    };
  }
};
...
/**
 * V11 Season Lifecycle Engine
 * Automated detection of the current phase based on user-provided dates.
 */
export const v11SeasonEngine = {
  calculatePhase(dates: {
    practice_start_date?: string;
    season_start_date?: string;
    playoff_start_date?: string;
    season_end_date?: string;
  }): 'Off-Season' | 'Pre-Season' | 'In-Season' | 'Playoffs' | 'Post-Season' {
    const now = new Date();

    const pStart = dates.practice_start_date ? new Date(dates.practice_start_date) : null;
    const sStart = dates.season_start_date ? new Date(dates.season_start_date) : null;
    const plyStart = dates.playoff_start_date ? new Date(dates.playoff_start_date) : null;
    const sEnd = dates.season_end_date ? new Date(dates.season_end_date) : null;

    if (sEnd && now > sEnd) return 'Post-Season';
    if (plyStart && now >= plyStart) return 'Playoffs';
    if (sStart && now >= sStart) return 'In-Season';
    if (pStart && now >= pStart) return 'Pre-Season';

    return 'Off-Season';
  },

  getPhaseProgress(dates: any, phase: string): number {
    const now = new Date().getTime();
    if (phase === 'Off-Season') return 0;
    if (phase === 'Post-Season') return 100;

    let start = 0;
    let end = 0;

    if (phase === 'Pre-Season') {
      start = new Date(dates.practice_start_date).getTime();
      end = new Date(dates.season_start_date).getTime();
    } else if (phase === 'In-Season') {
      start = new Date(dates.season_start_date).getTime();
      end = new Date(dates.playoff_start_date).getTime();
    } else if (phase === 'Playoffs') {
      start = new Date(dates.playoff_start_date).getTime();
      end = new Date(dates.season_end_date).getTime();
    }

    if (!start || !end) return 50; // Fallback
