...
export function determineRecommendation(text: string, mood: string, sport: string = 'Hockey', isGameday: boolean = false, coachNotes?: string): ExpertRule['recommendation'] {
    ...
    if (isGameday) {
        if (mood === 'happy' || mood === 'neutral') {
            return {
                ...
                drill: { name: "Hand-Eye Activation", duration: "5 mins", type: "physical" },
                videoWait: 0
            };
        } else {
            ...
            return {
                ...
                drill: { name: "Box Breathing", duration: "3 mins", type: "mental" },
                videoWait: 0
            };
        }
    }
    ...
    if (coachNotes && coachNotes.length > 5) {
        return {
            ...
            drill: { name: "Coach Assigned Specifics", duration: "As Assigned", type: "physical" },
            videoWait: 0
        };
    }
