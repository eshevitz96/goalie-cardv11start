...
import { User, ChevronRight, CheckCircle, Calendar } from 'lucide-react';
    ...
    useEffect(() => {
        if (editItem) {
            ...
            setFormData({
                ...
                backupEmail: (editItem.raw_data as any)?.backup_email || "", // Load from raw_data
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const payload = {
                ...
                raw_data: {
                    ...formData.rawData,
                    goalie_email: formData.athleteEmail,
                    backup_email: formData.backupEmail // Save to raw_data
                }
            };
                    ...
                    {step === 2 && (
                        <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                            ...
                            <div className="p-4 bg-indigo-500/5 border border-indigo-500/10 rounded-xl space-y-3">
                                <h4 className="text-xs font-bold text-indigo-400 uppercase tracking-wider flex items-center gap-2"><GoalieGuardLogo size={14} /> Guardian Info</h4>
                                <Input value={formData.parentName} onChange={e => setFormData({ ...formData, parentName: e.target.value })} placeholder="Guardian Name" />
                                <Input type="email" value={formData.guardianEmail} onChange={e => setFormData({ ...formData, guardianEmail: e.target.value })} placeholder="Guardian Email (Primary)" />
                                <div className="pt-2 border-t border-indigo-500/10">
                                    <label className="text-xs text-muted-foreground mb-1.5 block">Backup / Secondary Email</label>
                                    <Input
                                        type="email"
