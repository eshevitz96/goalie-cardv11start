...
export async function createEvent(rosterId: string, eventData: { 
    name: string, 
    ...
    try {
        ...
        const { data, error } = await supabaseAdmin.from('events').insert({
            ...
            scouting_clips: eventData.scouting_clips || []
        }).select().single();
...
export async function addNewGoalieToAccount(userId: string, goalieName: string, sport: string = 'Hockey', team?: string, gradYear?: number | null) {
...
export async function syncShotEvents(rosterId: string, eventId: string, shots: any[], journalData?: any) {
    ...
    try {
        ...
        const { data: gameSession, error: sessionError } = await supabaseAdmin
            ...
            .insert({
                ...
                soreness: journalData?.soreness || null,
                goal_focus: journalData?.goal_focus || null,
                journal_notes: journalData?.notes || null,
                status: 'completed',
...
// -----------------------------------------------------------------------------
// SOCIAL & CONNECTIVITY Actions (V11)
// -----------------------------------------------------------------------------

export async function followGoalie(targetGoalieId: string) {
    try {
        const supabase = clientSupabase;
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) return { success: false, error: "Not authenticated" };
...
export async function generateReferralLink(rosterId: string) {
    if (!rosterId) return { success: false, error: "Missing ID" };
    // In a real system, we'd hash this or use a vanity slug
    const referralCode = `GC-${rosterId.substring(0, 4)}-${Math.floor(Math.random() * 999)}`;
    return { success: true, link: `https://goaliecard.app/signup?ref=${referralCode}` };
}
...
export async function syncPerformanceIndex(userId: string, sourceType: string, sourceId: string) {
    ...
    try {
        ...
        // 1. Fetch Context (Roster Status & Readiness)
        ...
        // 2. Fetch Performance & Discipline Metrics
        ...
        // 3. Simple Streak Calculation for Backend Logic
        ...
        let activeStreak = 1; // Default floor
        if (recentSnaps && recentSnaps.length > 1) {
            // Simplified streak logic for server-side
            activeStreak = recentSnaps.length; 
        }

        // 4. Calculate New Score (V11 Pure Logic)
        ...
        // V11 Algorithm Dimensions (Stability, Execution, Readiness)
        const stabilityRaw = Math.min(100, (svPct / 0.95) * 100);
        const executionRaw = Math.min(100, (protocolCount || 0) * 10 + 50); 
        
        const streakRaw = Math.min(100, (activeStreak || 1) * 12);
        const readinessStatsRaw = ((10 - soreness) * 5) + (sleep * 5);
        const readinessRaw = (streakRaw * 0.6) + (readinessStatsRaw * 0.4);
        
        const rawWeightedTotal = (stabilityRaw * 0.5) + (executionRaw * 0.3) + (readinessRaw * 0.2);

        // 5. Apply Adaptive Exponent (The Feel)
 ...
 */
export async function completeProtocolSession(sessionId: string, userId: string, metadata: any = {}) {
    if (!sessionId || !userId) return { success: false, error: "Missing data" };

    try {
        const supabase = getSupabaseAdmin();

        // 1. Mark session as complete and store Neural Notes / Performance metadata
...
export async function getMentalTrainingHistory(userId: string) {
    ...
    try {
        ...
        const { data, error } = await supabase
            ...
            .eq('user_id', userId)
            .eq('status', 'complete')
            .like('template_id', 'mental-%')
            .order('completed_at', { ascending: false });

        if (error) throw error;
        return { success: true, history: data || [] };
    } catch (err: any) {
        console.error("Get Mental History Error:", err);
