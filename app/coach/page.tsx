...
export default function CoachDashboard() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // V11: Strictly query only assigned goalies for this coach
            let query = supabase
                .from('roster_uploads')
                .select('*');
            
            if (coachId) {
                // Use a complex OR filter to catch all assignment types
                const orFilter = `assigned_coach_id.eq.${coachId}${coachTeamId ? `,team_id.eq.${coachTeamId}` : ''}`;
                query = query.or(orFilter);
            }

            const { data: rawRosterData, error: rosterError } = await query;
            
            if (rosterError) console.error("[DASHBOARD_FETCH_ERROR]", rosterError);

            let rosterData: any[] = [];

            if (rawRosterData) {
                // Final safety pass for JSONB assigned_coach_ids if needed (Supabase .or doesn't support JSONB array contains easily yet)
